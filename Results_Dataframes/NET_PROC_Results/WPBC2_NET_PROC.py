...
for count, fname in enumerate(glob.glob(path)):  # Iterate through every dataset in NET_PROC
...
np.save('NET_PROC_wpbc2_roc_fpr', np.array(wpbc2_fpr_list, dtype=object))
np.save('NET_PROC_wpbc2_roc_tpr', np.array(wpbc2_tpr_list, dtype=object))
columns = ['WPBC2']
df = pd.DataFrame(wpbc2_list, columns=columns)
print(df)
df.to_csv('NET_PROC_WPBC2_AUC_Table.csv')
