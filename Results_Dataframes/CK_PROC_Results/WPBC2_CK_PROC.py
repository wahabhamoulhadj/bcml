...
for count, fname in enumerate(glob.glob(path)):  # Iterate through every dataset in CK_PROC
...
np.save('CK_PROC_wpbc2_roc_fpr', np.array(wpbc2_fpr_list, dtype=object))
np.save('CK_PROC_wpbc2_roc_tpr', np.array(wpbc2_tpr_list, dtype=object))
columns = ['WPBC2']
df = pd.DataFrame(wpbc2_list, columns=columns)
print(df)
df.to_csv('CK_PROC_WPBC2_AUC_Table.csv')
