...
df1 = pd.read_csv('CK_NET_all_auc.csv')
indexs = df1['Unnamed: 0']

df.drop(df.filter('Unnamed: 0'), axis=1, inplace=True)
df1.drop(df1.filter('Unnamed: 0'), axis=1, inplace=True)
print(df1)
roc_curves_tpr_list = np.load('CK_NET_all_six_models_tpr.npy', allow_pickle=True)
roc_curves_fpr_list = np.load('CK_NET_all_six_models_fpr.npy', allow_pickle=True)
bbc_fpr = np.load('CK_NET_bbc_roc_fpr.npy', allow_pickle=True)
bbc_tpr = np.load('CK_NET_bbc_roc_tpr.npy', allow_pickle=True)
# bbc_tpr = np.array(bbc_tpr)
# bbc_fpr = np.array(bbc_fpr)

ibc_fpr = np.load('CK_NET_ibc_roc_fpr.npy', allow_pickle=True)
ibc_tpr = np.load('CK_NET_ibc_roc_tpr.npy', allow_pickle=True)

pbc_fpr = np.load('CK_NET_pbc_roc_fpr.npy', allow_pickle=True)
pbc_tpr = np.load('CK_NET_pbc_roc_tpr.npy', allow_pickle=True)

wpibc_fpr = np.load('CK_NET_wpibc_roc_fpr.npy', allow_pickle=True)
wpibc_tpr = np.load('CK_NET_wpibc_roc_tpr.npy', allow_pickle=True)

wpbc2_fpr = np.load('CK_NET_WPBC2_roc_fpr.npy', allow_pickle=True)
wpbc2_tpr = np.load('CK_NET_WPBC2_roc_tpr.npy', allow_pickle=True)
...
for count in range(len(df)):
    ...
    plt.legend(["Naive Bayes AUC : {:.3f}".format(df.iloc[count][1]),
                ...
                "BBC2 AUC : {:.3f} ".format(pd.read_csv('CK_NET_BBC_AUC_Table.csv').iloc[count][1]),
                "IBC AUC : {:.3f} ".format(pd.read_csv('CK_NET_AUC_Table_IBC.csv').iloc[count][1]),
                "PBC AUC : {:.3f} ".format(pd.read_csv('CK_NET_PBC_AUC_Table.csv').iloc[count][1]),
                "WPBC2 AUC : {:.3f} ".format(pd.read_csv('CK_NET_WPBC2_AUC_Table.csv').iloc[count][1]),
                "WPIBC AUC : {:.3f} ".format(pd.read_csv('CK_NET_AUC_Table_WPIBC.csv').iloc[count][1]),], loc='lower right')

    plt.xlabel("FPR")
    plt.ylabel("TPR")
    plt.title(indexs[count])
    g = indexs[count][:-4]
    plt.savefig(r"CK_NET_ROC_Curves/ {}.png".format(g), format='png')

    # plt.show()
# directory = r'CK_NET_ROC_Curves'
