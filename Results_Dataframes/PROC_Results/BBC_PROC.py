...
np.save('PROC_bbc_roc_fpr', np.array(bbc_fpr_list, dtype=object))
np.save('PROC_bbc_roc_tpr', np.array(bbc_tpr_list, dtype=object))
columns = ['BBC']
df = pd.DataFrame(bbc_list, columns=columns)
print(df)
df.to_csv('PROC_BBC_AUC_Table.csv')
