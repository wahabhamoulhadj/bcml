...
pbc_fpr_list = []
pbc_tpr_list = []
for count, fname in enumerate(glob.glob(path)): #Iterate through every dataset in PROC
...
np.save('PROC_pbc_roc_fpr', np.array(pbc_fpr_list, dtype=object))
np.save('PROC_pbc_roc_tpr', np.array(pbc_tpr_list, dtype=object))
columns = ['PBC']
df = pd.DataFrame(pbc_list, columns=columns)
print(df)
df.to_csv('PROC_PBC_AUC_Table.csv')
