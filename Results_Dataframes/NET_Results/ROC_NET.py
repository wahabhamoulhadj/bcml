...
df1 = pd.read_csv('NET_all_auc.csv')
indexs = df1['Unnamed: 0']

df.drop(df.filter('Unnamed: 0'), axis=1, inplace=True)
df1.drop(df1.filter('Unnamed: 0'), axis=1, inplace=True)
print(df1)
roc_curves_tpr_list = np.load('NET_all_six_models_tpr.npy', allow_pickle=True)
roc_curves_fpr_list = np.load('NET_all_six_models_fpr.npy', allow_pickle=True)
bbc_fpr = np.load('NET_bbc_roc_fpr.npy', allow_pickle=True)
bbc_tpr = np.load('NET_bbc_roc_tpr.npy', allow_pickle=True)
...
wpbc2_fpr = np.load('NET_WPBC2_roc_fpr.npy', allow_pickle=True)
wpbc2_tpr = np.load('NET_WPBC2_roc_tpr.npy', allow_pickle=True)
...
for count in range(len(df)):
    ...
    g = indexs[count][:-4]
    plt.savefig(r"NET_ROC_Curves/ {}.png".format(g), format='png')

    # plt.show()
# directory = r'NET_ROC_Curves'
