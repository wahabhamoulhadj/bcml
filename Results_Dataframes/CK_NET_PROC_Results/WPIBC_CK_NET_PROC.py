...
columns = ['WPIBC']
np.save('CK_NET_PROC_wpibc_roc_fpr', np.array(WPibc_fpr, dtype=object))
np.save('CK_NET_PROC_wpibc_roc_tpr', np.array(WPibc_tpr, dtype=object))
df = pd.DataFrame(WPibc_list, columns=columns)

df.to_csv('CK_NET_PROC_AUC_Table_WPIBC.csv')
