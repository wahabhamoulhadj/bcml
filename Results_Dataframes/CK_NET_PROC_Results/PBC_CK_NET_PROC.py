...
np.save('CK_NET_PROC_pbc_roc_fpr', np.array(pbc_fpr_list, dtype=object))
np.save('CK_NET_PROC_pbc_roc_tpr', np.array(pbc_tpr_list, dtype=object))
columns = ['PBC']
df = pd.DataFrame(pbc_list, columns=columns)
print(df)
df.to_csv('CK_NET_PROC_PBC_AUC_Table.csv')
